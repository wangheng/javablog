...
CKEDITOR.dialog
		.add(
				'checkspell',
				function(a) {
					var b = CKEDITOR.tools.getNextNumber(), c = 'cke_frame_'
							+ b, d = 'cke_data_' + b, e = 'cke_error_' + b, f, g = document.location.protocol
							...
							+ '"'
							+ ' allowtransparency="1">'
							+ '</iframe>', j = a.config.wsc_customLoaderScript
							|| g
							+ '//loader.webspellchecker.net/sproxy_fck/sproxy.php'
							+ '?plugin=fck2' + '&customerid='
					...
					window._cancelOnError = function(m) {
						if (typeof window.WSC_Error == 'undefined') {
							CKEDITOR.document.getById(c).setStyle('display',
									'none');
					...
					;
					return {
						title : a.config.wsc_dialogTitle
								|| a.lang.spellCheck.title,
						minWidth : 485,
						minHeight : 380,
						...
						onShow : function() {
							var m = this
									...
									.getElement();
							m.setHtml(i);
							m.getChild(2).setStyle('height',
									this._.contentSize.height + 'px');
							if (typeof window.doSpell != 'function')
								CKEDITOR.document
										.getHead()
										.append(
						...
						contents : [ {
							id : 'general',
							label : a.config.wsc_dialogTitle
									|| a.lang.spellCheck.title,
							padding : 0,
							elements : [ {
								type : 'html',
